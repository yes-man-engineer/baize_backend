...
1. **预算必须问到，但不固定在第几问** — 这个数字决定整份方案和每条候选路径的
   大小。模型自己挑时机问，`extracted.budget` 抽出来回填，`parseMoney` 只做
   带单位的兜底。
   问法只能是投入（"打算先投多少钱进去"），prompt 里明令禁止出现
   "亏""打水漂""赔"这类字眼，也不许让用户去设想失败。
   一上来就问人家能亏多少，晦气，而且浇灭刚冒头的那点兴头。
...
**建项目和开场白是两个请求**：`POST /projects` 只落库，立刻返回 token；
开场白要等模型 8 到 20 秒，单独走 `POST /projects/:token/opening`。
前端拿到 token 马上跳转，等待显示在对话页里，带「已思考 N 秒」的计时。
搁在一个请求里的话用户要对着首页的按钮干等二十秒，什么反馈都没有。
`next_action=opening` 就是告诉前端还差这一句，重复调只会拿回同一句。

**入口选错了会自动纠偏**：有人会在入口 A 的框里打"不知道干啥，随便给点建议"。
第一轮回答之后，主干提问的返回里带一个 `idea_too_vague`，判定为真就当场改走
`scouting`，并清掉 `Idea`（那不是一个想法，留着会污染后面的 prompt）。
只认第一轮，后面几轮聊的是城市、时间、手上有什么，再翻盘会把人在两条路之间来回甩。

判断折在已有的那次模型调用里，没有额外开销。
